    ...
    public class DisputasController : ControllerBase
    {
        ...
        public async Task<IActionResult> DisputaEmGrupoAsync(Disputa d)
        {
            try
            {
                ...
                {
                    List<Personagem> atacantes = personagens.Where(personagens => personagens.PontosVida > 0).ToList();
                    ...
                    if(qtdePersonagensVivos == 1)
                    {
                        string resultadoFinal = $"{atacante.Nome.ToUpper()} é CAMPEÃO com {atacante.PontosVida} pontos de vidas restantes!";
                        d.Narracao += resultado;
                        d.Resultados.Add(resultado);
